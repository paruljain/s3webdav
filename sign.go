...
func (b *Bucket) sign(request *http.Request) {
	...
	if v, ok := params["Expires"]; ok {
		...
		expires = true
		date = v[0]
		params["AWSAccessKeyId"] = []string{b.AccessKey}
	}
	...
	if len(sarray) > 0 {
		sort.Sort(sarray)
		canonicalPath = canonicalPath + "?" + url.QueryEscape(strings.Join(sarray.StringSlice(), "&"))
	}

	payload := request.Method + "\n" + md5 + "\n" + ctype + "\n" + date + "\n" + xamz + canonicalPath
	hash := hmac.New(sha1.New, []byte(b.SecretKey))
	...
	if expires {
		...
		request.Header["Authorization"] = []string{"AWS " + b.AccessKey + ":" + string(signature)}
	}
	if debug {
